...
public class InventoryReport extends HttpServlet {
    ...
    */
    ...
    protected void DisplayInventory(HttpServletRequest request,HttpServletResponse response)
    throws ServletException, java.io.IOException {
		...
		pw.println("<span style='color:blue;'>"+"Inventory Report</span></h1> ");
		if(request.getAttribute("checkMsg") != null )
		{
			pw.println("<h2 style='color:white;background-color:blue;'>"+(String)(request.getAttribute("checkMsg"))+"</h2>");
		...
		else
		{
			...
			pw.println("<td>Inventory Quantity</td>");
			pw.println("</tr>");
			for(ProductDetails item : items)
			{
				pw.println("<tr><td>"+j+"</td>");
				j++;
				pw.println("<td>"+item.getProductName()+"</td>");
				pw.println("<td>"+item.getPrice()+"</td>");
				pw.println("<td>"+MySQLDataStoreUtilities.GetProductQuantity(item.getProductName())+"</td>");
				pw.println("</tr>");
			}
		}
		pw.println("</table>");
